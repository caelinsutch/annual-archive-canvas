...
export async function resolveReport(report) {
  ...
  if (!zip) return { kind: "external", pages: [], source: report.s };
  const stem = zip.name.replace(/_jp2\.zip$/, "");
  const scan = files.find((f) => f.name === stem + "_scandata.xml");
  if (!scan) return { kind: "external", pages: [], source: report.s };
