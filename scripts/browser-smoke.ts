...
try{
 ...
 await browser('eval',`[...document.querySelectorAll('#page-canvas button')].find(button=>button.tabIndex===0).click()`);
 await until(`document.querySelector('#reader.reading-page')&&!document.querySelector('.page-transition')`);
 assert.equal(await evaluate(`document.querySelector('#page-canvas').inert`),true);
 assert.notEqual(await evaluate(`getComputedStyle(document.querySelector('#page-reader')).backdropFilter`),'none');
 assert.ok(await evaluate(`document.querySelector('#page-reader img').getBoundingClientRect().height<=innerHeight-150`));checks.push('Page expands into fitted reader over a blurred, inert canvas');
 await browser('screenshot','outputs/refined-page-focus.png');await browser('press','Escape');
 await until(`!document.querySelector('#reader.reading-page')&&!document.querySelector('.page-transition')`);assert.equal(await evaluate(`document.querySelector('#reader').open`),true);checks.push('Escape returns the page to its canvas');
