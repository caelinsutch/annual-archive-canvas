...
export class ArchiveCanvas {
  constructor(root, { items = [], pages = false, onSelect, onZoom }) {
    ...
    root.innerHTML =
      '<canvas class="webgl" aria-hidden="true"></canvas><div class="canvas-cards"></div>';
    this.layer = root.lastElementChild;
    try {
  ...
  create(key, item) {
    ...
    picture.className = "artifact-paper";
    ...
    img.onerror = () => {
      ...
      picture.textContent = this.pages ? "Page scan unavailable" : item.o;
    };
  ...
  tick() {
    ...
    const ease = this.reduced ? 1 : 0.11;
    ...
    for (let row = startR; row <= endR; row++)
      for (let col = startC; col <= endC; col++) {
        ...
        c.el.style.setProperty("--hover", c.hover);
        if (c.mesh) {
